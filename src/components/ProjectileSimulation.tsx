...
function ProjectileSimulation() {
    ...
    const scale = basescale / viewScale; // Adjusted scale factor
    ...
    const toCanvasX = (x) => (x * scale) % width;
    ...
    const handleAngleChange = (newAngle) => {
        setParams(prev => ({ ...prev, angle: newAngle[0] }));
    };

    const handleVelocityChange = (newVelocity) => {
        setParams(prev => ({ ...prev, velocity: newVelocity[0] }));
    };
    ...
    return (
        <div className="flex flex-col items-center w-full p-4 animate-fade-in">
            <div className="w-full mb-4 flex flex-wrap items-center justify-between">
                <h2 className="text-xl font-bold">Projectile Motion Simulation</h2>
                ...
                <div className="flex space-x-2 mt-2 sm:mt-0">
                    <button 
                        ...
                        className={`px-3 py-1 rounded-md ${showTrail ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-gray-700'}`}
                    >
                        {showTrail ? 'Hide Trail' : 'Show Trail'}
                    </button>
                    <button 
                        onClick={() => setAutoScale(!autoScale)}
                        className={`px-3 py-1 rounded-md ${autoScale ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-gray-700'}`}
                    >
            ...
            <div className={`relative w-full h-64 md:h-80 rounded-lg shadow-lg overflow-hidden ${environments[environment].background}`}>
                <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="mx-auto">
                    ...
                    {/* Trajectory path - ensuring it stays within canvas */}
                    {showTrail && trajectory.length > 1 && (
                        <polyline
                            ...
                            fill="none"
                            stroke={environment === 'earth' ? "rgba(99, 102, 241, 0.6)" : 
                    ...
                    {/* Projectile object - ensure it's visible within canvas */}
                    ...
                    <CardContent className="pt-6">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="bg-gray-100 dark:bg-gray-800 p-3 rounded-md">
                                <p className="text-sm font-medium">Maximum Height</p>
                            ...
                            <div className="bg-gray-100 dark:bg-gray-800 p-3 rounded-md">
                                <p className="text-sm font-medium">Current X</p>
                                <p className="text-lg font-bold">{position.x.toFixed(2)} m</p>
                            </div>
                            <div className="bg-gray-100 dark:bg-gray-800 p-3 rounded-md">
                                <p className="text-sm font-medium">Current Y</p>
                                <p className="text-lg font-bold">{Math.max(0, position.y).toFixed(2)} m</p>
                            </div>
                        </div>
                    </CardContent>
                </Card>
